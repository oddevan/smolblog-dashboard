...
{#if items && items.length > 0}
	<div
		class="mb-5 overflow-x-scroll text-sm font-medium text-center text-gray-500 dark:text-gray-400"
	>
			...
			{#each items as item}
				...
				<li>
					<a
						...
						aria-current={$page.url.pathname === path ? 'page' : undefined}>{title}</a
					>
				</li>
			{/each}
		</ul>
